...
const itemsPerSlide = 3; // Nombre d'items à montrer par slide
let index = 0;

function showNextTestimonial() {
    index++;
    if (index >= Math.ceil(totalItems / itemsPerSlide)) {
        index = 0; // Revenir au premier slide à la fin
    }
    carousel.style.transform = `translateX(-${index * (100 / itemsPerSlide)}%)`;
}

function showPrevTestimonial() {
    index--;
    if (index < 0) {
        index = Math.ceil(totalItems / itemsPerSlide) - 1; // Aller au dernier slide au début
    }
    carousel.style.transform = `translateX(-${index * (100 / itemsPerSlide)}%)`;
}

// Ajouter une fonction pour faire défiler automatiquement toutes les 4 secondes
...
// Initial call to display the first set of items
